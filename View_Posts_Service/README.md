...
1. Curl command for POST: 
```curl -X POST -d '{"url": "https://www.umass.edu"}' -H "Content-Type: application/json" http://localhost:5000/link/shorten```

2. Link for testing redirect of short link: [http://localhost:5000/short_url](http://localhost:5000/short_url) - short_link will be returned in the terminal and can be copied into the browser. Replace short_link with the generated short link.

3. Getting an HTML: [http://localhost:5000/mapping/html](http://localhost:5000/mapping/html)

4. Getting a JSON: [http://localhost:5000/mapping/json](http://localhost:5000/mapping/json)
